function DrawrBrushes(onload_continuation){
	...
	this.brush_variations = [this.named_colors.length, 6, 0, 0]; //102];
	this.custom_index = 4;
		...
		for (var j = 0; j<this.brush_variations[i]; ++j){
			...
			if (type == "brush"){
				...
				for (var k = 0; k<this.size_variations.length;++k){
					...
					temp_img.src = "slime/brushes/"+name+"/"+size+".png"; //brushes/circle/0/16.png
				...
				var temp_img = new Image();
                this.count_to_load++; // new image to load
				temp_img.src = "slime/brushes/" + name + "/" + j + ".png";
...
DrawrBrushes.prototype.addCustomStamp = function(image_src){
	...
	var brush_obj = {
		img: image, 
		name: brush_name, 
		path: "slime/brushes/custom/" + index + ".png",
		sized_images: null,
		color: {r: 255, g: 255, b: 255},
		type: "stamp", 
		loaded: 1
...
DrawrBrushes.brushToPath = function(brush, size){
    if(brush.type == "brush"){
        return "slime/brushes/"+brush.name+"/"+size+".png";
    }else{
        return brush.path;
    }
}
